        ...
        impl #name {
            ...
            pub fn machine() -> &'static elastic_core::tiers::TierMachine {
                ...
                MACHINE.get_or_init(|| {
                    ...
                    #(
                        tiers.push(Tier { name: #tier_lits, rank: #ranks });
                    )*
                    // Rebuild each declared transition endpoint from the tier
                    // list by exact name (so ranks and identity are correct).
                    let mut transitions = Vec::with_capacity(#edge_count);
                    #(
                        transitions.push(TierTransition {
                            from: Tier {
                                name: #from_lits,
                                rank: tiers.iter().find(|t| t.name == #from_lits).map(|t| t.rank).unwrap_or(0),
                            },
                            to: Tier {
                                name: #to_lits,
                                rank: tiers.iter().find(|t| t.name == #to_lits).map(|t| t.rank).unwrap_or(0),
                            },
                        });
                    )*
